...
try {
    // --- INSERT KE table 'reservations' ---
    // Tambah column 'location' dan tanda soal (?) ke-5
    $stmt_res = $conn->prepare("INSERT INTO reservations (person_id, reserve_date, return_date, reason, location, created_at, priority) VALUES (?, ?, ?, ?, ?, NOW(), ?)");
    
    // Urutan Bind: i (person_id), s (reserve), s (return), s (reason), s (location), i (priority)
    $stmt_res->bind_param("issssi", $user_id, $reserve_date_context, $return_date_context, $request_reason, $location, $priority);
    $stmt_res->execute();
    $reserve_id = $conn->insert_id;
    $stmt_res->close();

    // --- INSERT KE table 'reservation_items' ---
    ...
    $log_details = "User $user_name submitted reservation #$reserve_id at $location containing $item_count items.";
    
    log_activity($conn, 'user', $user_id, "Submit Reservation", $log_details);

    // --- 7. NOTIFICATIONS & EMAIL ---
    $tech_role_id = 2; 
    $notification_message = "New Reservation (#{$reserve_id}) at {$location} from {$user_name} requires approval.";
