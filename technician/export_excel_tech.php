...
session_start();
// Pastikan anda menetapkan lokasi yang betul untuk config.php
include '../config.php';

// Pastikan pelaporan ralat MySQL diaktifkan
...
// Ambil nama pengguna (Teknisi) - ini tidak digunakan dalam laporan CSV tetapi baik untuk pengesahan sesi
...
// --------------------------------------------------------
// PENGENDALIAN PARAMETER LAPORAN
// --------------------------------------------------------
...
ORDER BY r.return_date DESC, a.asset_code ASC"; // Diubah untuk menyusun mengikut tarikh kembali


$stmt_report = $conn->prepare($sql_report);
if ($stmt_report === false) { die("SQL Prepare Error: " . htmlspecialchars($conn->error)); }


// --------------------------------------------------------
// PENGENDALIAN BIND PARAMETER (PENTING UNTUK MENGGUNAKAN REFERENCES)
// --------------------------------------------------------

$bind_params_select = array();
$bind_params_select[] = $param_types_report;
// Bind parameter dengan rujukan (&)
for ($i = 0; $i < count($param_values_report); $i++) {
    $bind_params_select[] = &$param_values_report[$i];
}

// Mengikat parameter secara dinamik
...
fputcsv($output, array(
    ...
    'Asset Code', 
    'Item Type', 
    'Reservation Date', // Label diubah untuk kejelasan
    'Asset Return Date', 
    'Return Condition', 
    'Approved By', 
    'Checked Out By',
...
while ($row = $result->fetch_assoc()) {
    fputcsv($output, array(
        $row['user_name'],
        $row['item_name'],
        $row['asset_code'] ?? 'N/A', // Aset Code boleh NULL jika item tidak memerlukan aset bernombor siri
        $row['category_name'],
        $row['reserve_date'],
        $row['return_date'],
