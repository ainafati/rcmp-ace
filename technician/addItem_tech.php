...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['add_item_type_and_units'])) {
    ...
    if ($stmt->execute()) {
        ...
        if (isset($_POST['enable_manual_code']) && isset($_POST['manual_codes'])) {
            ...
            // Ambil 2 digit terakhir tahun (contoh: 2026 jadi 26)
            $year_short = date('y'); 
            
            // Padded Item ID kepada 3 digit (001 - 999)
            $padded_item_id = str_pad($last_item_id, 3, '0', STR_PAD_LEFT);

            for ($i = 1; $i <= $qty; $i++) {
                // Padded Sequence kepada 3 digit (001 - 999)
                $padded_qty = str_pad($i, 3, '0', STR_PAD_LEFT);
                
                // Gabung semua: Contoh 26 + 001 + 001 = 26001001 (8 digit)
                $auto_code = $year_short . $padded_item_id . $padded_qty;
...
/* Nav: Kurangkan margin atas */
.sidebar-nav {
    margin-top: 10px !important; /* Kawal jarak antara header dengan menu pertama */
...
<div class="sidebar" id="admin-sidebar">
    <div class="sidebar-header">
        ...
        <div class="logo-text"><strong>UniKL Technician</strong><br><span style="font-size: 0.8rem; color: #64748b;">System Support</span></div>
    </div>
